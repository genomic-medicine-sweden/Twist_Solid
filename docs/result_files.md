...
| **CNVs** | :~~: | ~~ |
|  **`results/dna/cnv/{sample}_{type}/{sample}_{type}.pathology_purecn.cnv.html`** | html | Interactive html report of the filtered CNV results <br />using combined tumor content estimated by a pathologist and purecn|
...
| **Fusions** | :~~: | ~~ |
|  **`results/dna/{sample}_{type}/fusion/{sample}_{type}.fuseq_wes.report.csv`** | table | Filtered report of DNA fusions from FuseqWES |
|  **`results/dna/{sample}_{type}/fusion/{sample}_{type}.juli.filtered.fusions.txt`** | table | Filtered report of DNA fusions from JuLI |
...
|  `results/dna/{sample}_{type}/biomarker/{sample}_{type}.pathology_purecn.scarhrd_cnvkit_score.txt` | table | HRD score <br />**OBS! experimental** |
| **QC** | :~~: | ~~ |
